...
var Kronic = (function() { 
  ...
  function parseExactDateParts(rawDay, rawMonth, rawYear, today) {
    ...
    if (rawYear) {
      if (rawYear.match(NUMBER)) {
        year = rawYear.length < 3 ? parseAmbiguousCentury(rawYear*1, today) : rawYear*1;
      } else {
        year = null;
      }
    } else
  ...
  function parseAmbiguousCentury(year_number, today) {
    var year_number = Math.floor((today.getYear()+1900)/100)*100+year_number;
    if (year_number > today.getYear()+1900) { year_number -= 100; }
    return year_number;
  }
